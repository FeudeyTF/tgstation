/datum/ntcode/node/statement/if_operator
	var/datum/ntcode/node/expression/condition
	var/datum/ntcode/node/block/block
	var/datum/ntcode/node/block/else_block

/datum/ntcode/node/statement/if_operator/New(datum/ntcode/node/expression/condition, datum/ntcode/node/block/block, datum/ntcode/node/block/else_block)
	. = ..()
	src.condition = condition
	src.block = block
	src.else_block = else_block

/datum/ntcode/node/statement/if_operator/analyze(datum/ntcode/analyzer/analyzer)
	condition.analyze(analyzer)
	block.analyze(analyzer)
	if(else_block)
		else_block.analyze(analyzer)

/datum/ntcode/node/statement/if_operator/evaluate(datum/ntcode/interpreter/interpreter)
	if(condition.evaluate(interpreter))
		return block.evaluate(interpreter)
	else if(else_block)
		return else_block.evaluate(interpreter)

/datum/ntcode/node/statement/if_operator/to_string(indent)
	var/result =  "[indent]if([condition.to_string("")]) [block.to_string(indent)]"
	if(else_block)
		result += "[indent]else [else_block.to_string(indent)]"
	return result

/datum/ntcode/node/statement/if_operator/parse(datum/ntcode/parser/parser)
	if(!parser.match("if"))
		return

	parser.skip("(")

	var/condition = GLOB.ntcode_expression_node.parse(parser)
	if(!condition)
	...
	if(parser.match("else"))
		else_block = GLOB.ntcode_block_node.parse(parser)
		if(!else_block)
			parser.error(new /datum/ntcode/error/parser/unexpected/construct/block(parser.current_token))
			return
...
/datum/ntcode/node/statement/while_operator
	var/datum/ntcode/node/expression/condition
	var/datum/ntcode/node/block/block

/datum/ntcode/node/statement/while_operator/New(datum/ntcode/node/expression/condition, datum/ntcode/node/block/block)
	. = ..()
	src.condition = condition
	src.block = block

/datum/ntcode/node/statement/while_operator/analyze(datum/ntcode/analyzer/analyzer)
	condition.analyze(analyzer)
	block.analyze(analyzer)

/datum/ntcode/node/statement/while_operator/evaluate(datum/ntcode/interpreter/interpreter)
	var/count = 0
...
/datum/ntcode/node/statement/while_operator/to_string(indent)
	return  "[indent]while([condition.to_string("")]) [block.to_string(indent)]"

/datum/ntcode/node/statement/while_operator/parse(datum/ntcode/parser/parser)
